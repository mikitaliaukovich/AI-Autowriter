...
1. ОЧИСТКА. Убирай из распознанной речи:
   ...
   - повторы и фальстарты: если автор начал фразу и тут же поправил себя, оставь \
ТОЛЬКО исправленный вариант, а первую попытку и слова отрицания выброси целиком. \
Например, «она отвернулась, нет, не отвернулась, а закрыла глаза» → «она закрыла глаза»;
...
- ВАЖНО: если P0 — реплика диалога (начинается с тире), то описание, действие или \
новая мысль — ВСЕГДА отдельный абзац. Внутрь реплики дописываются только слова автора \
об этой же реплике («— …, — сказал он.»).
...
FEW_SHOT: list[dict[str, str]] = [
    ...
    *_example(
        "P-1: — Ты уверен? — спросил он.\nP0: — Совершенно уверен.⟦КУРСОР⟧",
        "э-э ну она отвернулась нет не отвернулась а закрыла глаза и медленно выдохнула точка",
        {
            "mode": "dictation",
            "ops": [
                {
                    ...
                    "id": "P0",
                    "paragraphs": [
                        {"text": "Она закрыла глаза и медленно выдохнула.", "style": "normal"}
                    ],
                }
            ],
            "note": "описание после реплики — отдельный абзац; фальстарт убран",
        },
    ),
    *_example(
